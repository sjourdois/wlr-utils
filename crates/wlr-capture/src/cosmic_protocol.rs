//! Client bindings for the two COSMIC protocol extensions the focus backend needs.
//!
...
//! Its XML lives in [pop-os/cosmic-protocols], published on crates.io as
//! `cosmic-protocols` — but that crate is GPL-3.0-only, and wlr-utils is MIT OR
//! Apache-2.0, so the protocol files themselves (permissively licensed, see their
//! `<copyright>` headers) are vendored under `protocols/` and generated here instead.
//!
//! `cosmic-workspace-unstable-v1` is generated only because two deprecated
//! `zcosmic_toplevel_handle_v1` events carry a `zcosmic_workspace_handle_v1`
//! argument; nothing in the crate uses workspaces.
//!
