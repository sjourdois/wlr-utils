...
daemon-not-running = Демон wlr-switcher не запущено, тож цей оверлей оплачує повний запуск (близько 90 мс). Запустіть його з автозапуску сеансу: `wlr-switcher --daemon`.
