...
scratchpad-unsupported = У цього композитора немає scratchpad, між вікнами якого можна перемикатися, тому --scratchpad не можна застосувати. Для цього потрібен IPC sway ($SWAYSOCK). Запустіть `wlr-peek doctor`, щоб побачити, чи виявлено бекенд фокуса.
scratchpad-empty = У scratchpad немає жодного вікна. Немає чого показати.
scratchpad-not-put-aside = Композитор не повернув вікно у фокусі до scratchpad.
...
daemon-not-running = Демон wlr-overlayd не запущено, тож цей оверлей оплачує повний запуск (близько 90 мс). Запускайте `wlr-overlayd` разом із сеансом, щоб він з'являвся миттєво, або передайте --no-daemon, якщо так і задумано — тоді це повідомлення зникне. Див. `wlr-overlayd --help` або README wlr-chooser.
...
hold-no-modifier = Режим утримання ввімкнено: перемикач перемикає, щойно не утримується жодна клавіша Alt чи Super, — одразу й нічого не показавши, якщо під час відкриття жодна не утримується. Приберіть --hold, щоб оверлей чекав, коли жодна не утримується.
