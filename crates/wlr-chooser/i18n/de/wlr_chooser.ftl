...
scratchpad-unsupported = Dieser Compositor hat keinen Scratchpad, zwischen dessen Fenstern gewechselt werden könnte, daher kann --scratchpad nicht angewendet werden. Dafür wird die IPC von sway benötigt ($SWAYSOCK). Führe `wlr-peek doctor` aus, um zu sehen, ob ein Fokus-Backend erkannt wurde.
scratchpad-empty = Der Scratchpad enthält kein Fenster. Nichts anzuzeigen.
scratchpad-not-put-aside = Der Compositor hat das fokussierte Fenster nicht in den Scratchpad zurückgelegt.
...
daemon-already-running = Ein wlr-overlayd-Daemon läuft bereits.
daemon-none = Es läuft kein wlr-overlayd-Daemon.
daemon-gone = Der wlr-overlayd-Daemon hat sich ohne Antwort beendet.
daemon-not-running = Es läuft kein wlr-overlayd-Daemon, daher zahlt dieses Overlay den vollen Start (etwa 90 ms). Starte `wlr-overlayd` mit deiner Sitzung, damit es sofort erscheint, oder übergib --no-daemon, wenn du das so willst — dann verschwindet dieser Hinweis. Siehe `wlr-overlayd --help` oder die wlr-chooser-README.
