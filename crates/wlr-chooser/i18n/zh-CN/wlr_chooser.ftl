...
scratchpad-unsupported = 此合成器没有可供切换的 scratchpad，因此无法应用 --scratchpad。这需要 sway 的 IPC（$SWAYSOCK）。运行 `wlr-peek doctor` 查看是否检测到焦点后端。
scratchpad-empty = scratchpad 中没有窗口。没有可显示的内容。
scratchpad-not-put-aside = 合成器没有把当前聚焦的窗口放回 scratchpad。
...
daemon-not-running = 没有正在运行的 wlr-overlayd 守护进程，因此本次浮层要承担全部启动开销（约 90 毫秒）。随会话启动 `wlr-overlayd` 即可瞬间显示；若这正是你想要的，请加上 --no-daemon，本提示也会消失。参见 `wlr-overlayd --help` 或 wlr-chooser 的 README。
...
hold-no-modifier = 按住切换已开启：一旦没有按住 Alt 或 Super 键，切换器就会切换——若打开时没有按住任何一个，则立即切换，不显示任何内容。去掉 --hold，浮层就会在没有按住任何键时保持等待。
