...
hold-no-modifier = Режим удержания включён: переключатель переключает, как только не удерживается ни Alt, ни Super, — сразу и ничего не показав, если при открытии ни одна не удерживается. Передайте --no-hold, чтобы оверлей оставался открытым.
