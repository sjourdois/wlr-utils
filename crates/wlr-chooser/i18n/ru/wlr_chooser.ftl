...
daemon-not-running = Демон wlr-overlayd не запущен, поэтому этот оверлей оплачивает полный запуск (около 90 мс). Запустите его из автозапуска сеанса: `wlr-overlayd`.
