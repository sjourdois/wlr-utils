...
scratchpad-unsupported = У этого композитора нет scratchpad, между окнами которого можно переключаться, поэтому --scratchpad неприменим. Для этого нужен IPC sway ($SWAYSOCK). Запустите `wlr-peek doctor`, чтобы узнать, обнаружен ли бэкенд фокуса.
scratchpad-empty = В scratchpad нет ни одного окна. Показывать нечего.
scratchpad-not-put-aside = Композитор не вернул окно в фокусе в scratchpad.
...
hold-no-modifier = Режим удержания включён: переключатель переключает, как только не удерживается ни Alt, ни Super, — сразу и ничего не показав, если при открытии ни одна не удерживается. Уберите --hold, чтобы оверлей ждал, когда ни одна не удерживается.
