...
daemon-already-running = wlr-overlayd のデーモンはすでに実行中です。
daemon-none = wlr-overlayd のデーモンは実行されていません。
daemon-gone = wlr-overlayd のデーモンが応答せずに終了しました。
daemon-not-running = wlr-overlayd のデーモンが実行されていないため、このオーバーレイは起動処理をすべて負担します（約 90 ミリ秒）。セッションと一緒に `wlr-overlayd` を起動すれば即座に表示されます。そうしないのが意図なら --no-daemon を付けてください。この通知も出なくなります。`wlr-overlayd --help` または wlr-chooser の README を参照。
daemon-busy = wlr-overlayd のデーモンはすでに別のオーバーレイを表示しているため、こちらはこのプロセスで表示し、起動処理をすべて負担します。
...
hold-no-modifier = ホールド切り替えが有効です。Alt または Super キーが押されていない瞬間に切り替わります。開いた時点でどちらも押されていなければ、何も表示せずにすぐ切り替わります。どちらも押されていないときにオーバーレイを待機させるには、--hold を外してください。
